...
const getTierLevel = (totalXP) => Math.min(Math.floor(totalXP / 100), 2);
const updateXPBars = () => {
  ...
  categories.forEach(category => {
    ...
    fill.style.width = `${Math.min(xp, 100)}%`;
...
const addXP = (category, difficulty) => {
  xpData[category].total += XP_VALUES[difficulty];
  xpData[category].levelXP += XP_VALUES[difficulty];
  if (xpData[category].levelXP >= 100) {
    xpData[category].levelXP = 0;
  }
