...
const updateXPBars = () => {
  ...
  categories.forEach(category => {
    ...
    const xp = xpData[category];
    fill.style.width = `${Math.min(xp, 100)}%`;  // Cap at 100%
  });
};

// Add XP to category and save to localStorage
const addXP = (category, difficulty) => {
  xpData[category] = (xpData[category] || 0) + XP_VALUES[difficulty];
...
const renderTasks = () => {
  ...
  if (categoryTasks.length === 0) {
    ...
    categoryTasks.forEach(task => {
      ...
      // Create the label and checkbox elements for each task
      ...
      // Add an event listener to toggle task completion status and add XP
      checkbox.addEventListener("change", () => {
        if (!task.completed) {  // Only add XP if task was incomplete
